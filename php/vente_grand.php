...
//numero_vent 	email 	client 	date_vent 	nb_produit 	total 	reduction 	pour_paye 	credit_cache
//client  date totalprod drone totalevene redvene adonner
if(isset($_POST['client']))$client = $_POST['client'];else$client="le client n'été pas envoyé en POST Méthod";
...
if (isset($_POST['enregistrer1'])) {
	...
	elseif(empty($client)){echo("<div class='comment c' style='top:350px;'>pas de client à lui vendre!</div>");}
...
$query2="INSERT INTO ventes_grand (email,client,date_vent,nb_produit,total,reduction ,pour_paye, credit_cache)VALUES ('$email','$client','$date','$nbpro','$total','$reduction', '$payer','$ache')";
...
<br><br><br>
<h2> Les Ventes en gros </h2>
<br><div  class ='btn c'  onClick='aide()' > Aide à enregistrer une opération du vente en gros!!</div><br>


<form method="post" action="vente_grand.php" class="hr1"> 
<table class="table2">
	<tbody>
<tr>		<td><label class="indice">Vente en gros numero:</label></td>
				<?php
					$query13="SELECT MAX(numero_vent) from ventes_grand where email='$email' ORDER BY numero_vent DESC";
					$resut13=mysqli_query($connect,$query13);
					if (mysqli_num_rows($resut13) > 0) {
						 $rslt13=mysqli_fetch_array($resut13); 
						 $nu=(int)$rslt13['MAX(numero_vent)']+1;
						}
					echo"<td><input type='text' readonly name='num' style='left:0px;width:100px;' class='inputtext modprod' value=".$nu." >";
				?>
			</td>
			<td><label class="indice">Client:</label></td>
			<td>
				<select name='client' id='clien' class='selected select'>
		<option></option>
		<?php
		$query11="SELECT * from clients where email='$email' ";
		$resut11=mysqli_query($connect,$query11);
		while($rslt=mysqli_fetch_assoc($resut11)){
		echo"<option value=".$rslt['nom_client']." >".$rslt['nom_client']." </option>";
		}
		?>
		</select>
			</td>
...
<div id="aide">
		<h2>Aide à enregistrer une opération du vente en gros !!</h2>
		<br>
		<ul>
